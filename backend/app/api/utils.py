...
def estimate_eta_minutes(distance_km: float, buffer_min: int | None = None) -> int:
...
def require_active_reservation(
    res_id: str, owner_id: str | None = None, allow_admin: bool = False
) -> dict[str, Any]:
    record = ensure_reservation_owner(DB.get_reservation(res_id), owner_id, allow_admin)
