...
class HealthChecker:
    ...
    async def check_all(self) -> dict[str, Any]:
        ...
        """
        gomap_configured = _is_configured(settings.GOMAP_GUID)
        sentry_configured = _is_configured(settings.SENTRY_DSN)
        auth0_configured = _is_configured(settings.AUTH0_DOMAIN)

        checks = {
            "database": await self._check_database(),
            "gomap": await self._check_gomap() if gomap_configured else {"status": "disabled"},
            "auth0": (
    ...
    async def _check_gomap(self) -> dict[str, Any]:
        """Check if GoMap API is reachable."""
        cache_key = "gomap"
        ...
        try:
            from .gomap import gomap_enabled

            if not gomap_enabled():
                result = {"status": "disabled", "reason": "GoMap not configured"}
                self._cache_check(cache_key, result)
                return result

            # Make a lightweight API call to check connectivity
            url = settings.GOMAP_BASE_URL
            async with httpx.AsyncClient(timeout=3.0) as client:
                response = await client.get(url)
                # GoMap ASMX service returns 200 even for GET on base URL
                is_reachable = response.status_code in {200, 404, 405}

            if is_reachable:
                result = {
                    "status": "ok",
                    "endpoint": settings.GOMAP_BASE_URL,
                }
            else:
                result = {
                    "status": "error",
