...
class BackupManager:
    ...
    def create_backup(self, description: str | None = None) -> Path:
        ...
        try:
            ...
            self._backup_file(
                source=settings.data_dir / "reservations.json",
                dest=backup_path / "reservations.json",
            )
    ...
    def _backup_file(self, source: Path, dest: Path) -> None:
    ...
    def _restore_files(self, backup_dir: Path) -> None:
        ...
        # Restore reservations.json
        reservations_backup = backup_dir / "reservations.json"
        if reservations_backup.exists():
            shutil.copy2(reservations_backup, settings.data_dir / "reservations.json")
