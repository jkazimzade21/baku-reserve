...
def _clear_all():
    r = SESSION.get("/reservations"); r.raise_for_status()
    for rec in r.json():
...
def test_restaurants_list_and_detail():
    r = SESSION.get("/restaurants"); r.raise_for_status()
    items = r.json()
    assert any(x["id"] == RID for x in items)
    r2 = SESSION.get(f"/restaurants/{RID}"); r2.raise_for_status()
    detail = r2.json()
...
def test_create_overlap_adjacency_and_cleanup():
    ...
    r = SESSION.post("/reservations", json=payload); assert r.status_code == 201
    rid1 = r.json()["id"]

    payload2 = dict(restaurant_id=RID, party_size=2, start=s2, end=e2, guest_name="E2", table_id=T2)
    r = SESSION.post("/reservations", json=payload2); assert r.status_code == 409

    payload3 = dict(restaurant_id=RID, party_size=2, start=s3, end=e3, guest_name="E3", table_id=T2)
    r = SESSION.post("/reservations", json=payload3); assert r.status_code == 201
    rid3 = r.json()["id"]
...
@pytest.mark.parametrize("bad_payload", [
    ...
    {"restaurant_id": "NOT-A-UUID", "party_size": 2, "start": "2025-10-23T10:00:00", "end": "2025-10-23T11:30:00", "guest_name":"X"},
    {"restaurant_id": RID, "party_size": 2, "start": "BAD", "end": "BAD", "guest_name":"X"},
    {"restaurant_id": RID, "party_size": 0, "start": "2025-10-23T10:00:00", "end": "2025-10-23T11:30:00", "guest_name":"X"},
    {"restaurant_id": RID, "party_size": -1, "start": "2025-10-23T10:00:00", "end": "2025-10-23T11:30:00", "guest_name":"X"},
    {"restaurant_id": RID, "party_size": 2, "start": "2025-10-23T11:30:00", "end": "2025-10-23T10:00:00", "guest_name":"X"},
])
...
def test_unknowns_and_capacity():
    day = _today()
    s, e = _iso(day, "12:00")
    assert SESSION.get(f"/restaurants/00000000-0000-0000-0000-000000000000").status_code in (404,422)
    r = SESSION.post("/reservations", json=dict(
...
def test_autopick_min_capacity_choice():
    ...
    s, e = _iso(day, "13:00")
    r = SESSION.post("/reservations", json=dict(
        restaurant_id=RID, party_size=2, start=s, end=e, guest_name="AutoPick"
    ))
...
def test_prop_book_then_free_after_delete(t):
    ...
    before = SESSION.get(f"/restaurants/{RID}/availability", params={"date":day, "party_size":2}).json()
    slot_before = next(x for x in before["slots"] if x["start"] == s)

    r = SESSION.post("/reservations", json=dict(
        restaurant_id=RID, party_size=2, start=s, end=e, guest_name="Prop"
    ))
    assert r.status_code == 201
    rid = r.json()["id"]

    after = SESSION.get(f"/restaurants/{RID}/availability", params={"date":day, "party_size":2}).json()
