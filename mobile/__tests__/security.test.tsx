...
// Mock utility functions if they don't exist
const mockSanitizeInput = (input: string): string => {
  return input.replace(/<script>|<\/script>|javascript:|onerror=/gi, '');
};
...
describe('Input Validation', () => {
  describe('XSS Prevention', () => {
    ...
    it('should handle nested attacks', () => {
      ...
      // Multiple passes to catch nested attacks
      for (let i = 0; i < 3; i++) {
        sanitized = mockSanitizeInput(sanitized);
      }
...
describe('Data Sanitization', () => {
  it('should sanitize user-generated content', () => {
    ...
    const sanitized = {
      name: mockSanitizeInput(userContent.name),
      comment: mockSanitizeInput(userContent.comment),
  ...
  it('should sanitize deep link parameters', () => {
    const parseDeepLinkParams = (url: string): Record<string, string> => {
      ...
      urlObj.searchParams.forEach((value, key) => {
        ...
        params[key] = mockSanitizeInput(value);
      });
