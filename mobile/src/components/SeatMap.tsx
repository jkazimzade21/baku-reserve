...
export default function SeatMap({
  ...
  const handleSelectTable = (table: TableDetail) => {
    ...
    const nextId = selectedId === table.id ? null : table.id;
    onSelect?.(nextId);
    setPreview(nextId ? { table, anchor: { x: 0, y: 0 } } : null);
  };
  ...
  const activeTable = preview?.table ?? (selectedId ? tables.find((t) => t.id === selectedId) ?? null : null);
