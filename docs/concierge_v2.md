...
- `match_reason` keys use lower-cased slug (or UUID fallback). Client maps chips against `slug ?? id`.
...
- Keep `backend/app/data/restaurants.json` synced with runtime store (script snippet in `codexinfo.md`).
