...
This harness maximises coverage by:
  ...
  • Ensuring persisted JSON state is written and cleaned up.
...
class HttpEndToEndSuite:
    ...
    def availability_and_reservations(self) -> str:
        ...
        if RESERVATIONS_PATH.exists():
            raw = json.loads(RESERVATIONS_PATH.read_text(encoding="utf-8"))
            self._assert(
                any(rec.get("guest_name") == "FullStack Tester" for rec in raw.get("reservations", [])),
                "Persisted reservations.json missing the new booking",
            )
