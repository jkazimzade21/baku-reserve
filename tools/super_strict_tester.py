...
from app.storage import RES_PATH  # type: ignore  # noqa: E402
...
class UltimateBackendAudit:
    ...
    def _reset(self) -> None:
        ...
        DB._load()  # type: ignore[attr-defined]  # reload JSON store
    ...
    def persistence_roundtrip(self) -> str:
        ...
        self._assert(raw.get("reservations"), "reservations.json should not be empty")

        DB._load()  # type: ignore[attr-defined]
        listing = self.client.get("/reservations")
        self._assert(any(item["guest_name"] == "PersistStrict" for item in listing.json()), "persisted reservation missing")
        return "reservations persist to JSON and survive reload"
