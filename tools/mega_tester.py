...
from app.storage import DB, RES_PATH  # type: ignore  # noqa: E402
...
class BackendScenarioSuite:
    ...
    Interacts with the in-memory DB and persisted JSON to guarantee clean state.
    """
    ...
    def _reset(self) -> None:
        ...
        RES_PATH.write_text(json.dumps({"reservations": []}, indent=2), encoding="utf-8")
        DB._load()  # type: ignore[attr-defined]  # reload persisted reservations
    ...
    def persistence_roundtrip(self) -> str:
        ...
        self._assert(raw["reservations"], "reservations.json should contain persisted data")
        DB._load()
